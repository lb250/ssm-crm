...
<script type="text/javascript">
	$(function(){
		$("#convertBtn").click(function () {
			var clueId='${clue.id}'
			var isCreateTransaction=$("#isCreateTransaction").is(":checked");
			...
			$.ajax({
				url:"workbench/clue/convertClueToOther.do",
				dataType:"json",
				data:{
					clueId:clueId,
					isCreateTran:isCreateTransaction,
					money:money,
		...
		$("#aSource").click(function () {
				$("#t_Body").html("");
				$("#i_activityName").val("");
				$("#searchActivityModal").modal("show");

		})
		$("#t_Body").on("click","input[type='radio']",function () {
		...
		$("#i_activityName").keyup(function () {
				var name=$("#i_activityName").val();
				$.ajax({
					url:"workbench/clue/selectActivityListForConvert.do",
					data:{
						"name":name
					},
					dataType:"json",
					...
					success:function (data) {
						...
						html+="<tr>"
						html+="<td><input activityName='"+obj.name+"' value='"+obj.id+"' type=\"radio\" name=\"activity\"/></td>"
						html+="<td>"+obj.name+"</td>"
		...
		<form>
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    ...
		    <select id="stage"  class="form-control">
		    	<option></option>
		    	<c:forEach items="${stageList}" var="state">
					<option value="${state.id}">${state.value}</option>
				</c:forEach>
		    </select>
		  </div>
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    <label for="activityName">市场活动源&nbsp;&nbsp;<a id="aSource" href="javascript:void(0);"  style="text-decoration: none;"><span class="glyphicon glyphicon-search"></span></a></label>
